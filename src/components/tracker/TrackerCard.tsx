...
export function TrackerCard({ tracker, onClick, onTogglePause, onDelete, onFetch, isFetching }: TrackerCardProps) {
  ...
  // Most recent priceInsights from any successful fetch
  const latestInsights: PriceInsights | undefined = [...successRecords]
    .reverse()
    .find(r => r.priceInsights)?.priceInsights;

  // Blended sparkline data: local prices + API price_history values
  const apiPrices = latestInsights?.price_history?.map(([, p]) => p) ?? [];
  const blendedPrices = [
    ...apiPrices.slice(0, Math.max(0, 20 - prices.length)), // fill from API if local sparse
    ...prices,
  ].slice(-30);
      ...
      <div className="p-4">
        ...
        <div className="flex items-end justify-between mb-2">
          <div className="min-w-0">
            {currentPrice ? (
              <>
                {/* Price + price level badge on same line */}
        ...
        {/* Card actions */}
        <div
          className="flex gap-2 mt-3 pt-3 border-t border-slate-100 dark:border-slate-700"
          onClick={e => e.stopPropagation()}
        >
          <button
            onClick={() => onFetch(tracker)}
            disabled={isFetching || isPaused}
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-40 transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-xs font-medium text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors ml-auto"
          >
            <Trash2 size={12} />
            Delete
          </button>
